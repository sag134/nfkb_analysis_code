...
function [err,ind,data_ref,data_query]= CalcError(data_ref,data_query,scaling)
    % INPUTS
    ...
    err = zeros(1,size(data_query,2)); % we will calculate an error for each point in the query set
    ind = zeros(1,size(data_query,2)); % the index of the ref point that minimizes the error
    for i = 1:size(data_query,2) % for each data point in the query set
        ...
        err(i) = min(out); % find the minimum distance
        ind(i) = find(out==err(i),1); % find the index of the ref point with the min distance
        if mod(i,50)==0
            toc
        end
    end
